...
struct device device_list[] = {
	...
	{"ATmega169"   ,   8192, 0x100,  1024,  512, DF_NO_EICALL|DF_NO_EIJMP|DF_NO_ELPM|DF_NO_ESPM},
	{"ATmega169A"  ,   8192, 0x100,  1024,  512, DF_NO_EICALL|DF_NO_EIJMP|DF_NO_ELPM|DF_NO_ESPM},
	{"ATmega169P"  ,   8192, 0x100,  1024,  512, DF_NO_EICALL|DF_NO_EIJMP|DF_NO_ELPM|DF_NO_ESPM},
	{"ATmega169PA" ,   8192, 0x100,  1024,  512, DF_NO_EICALL|DF_NO_EIJMP|DF_NO_ELPM|DF_NO_ESPM},
...
predef_dev(struct prog_info *pi)
{
	...
	for (i=0; (!i)||(device_list[i].name); i++) {
		...
		if (pi->pass==PASS_1) { /* Pass 1 */
			...
			if (i != j) {
				fprintf(stderr,"Constant %s changed value from %" PRId64 " in pass1 to %d in pass 2\n",temp,j,i);
				return (False);
			}
